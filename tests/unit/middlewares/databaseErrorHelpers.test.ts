...
describe('databaseErrorHelpers', () => {
  describe('isValidationError', () => {
    it('should return true for error with name ValidationError', () => {
      expect(dbHelpers.isValidationError({ name: 'ValidationError' })).toBe(true);
    });
  ...
  describe('getDuplicateKey', () => {
    it('should extract duplicate key from error message', () => {
      const err = { message: 'E11000 duplicate key error collection: test.users index: email_1 dup key: { email: "test@example.com" }' };
