...
import { getAccessToRoute, getAnswerOwnerAccess } from "../middlewares/authorization/authMiddleware";
...
import { createAnswerSchema, updateAnswerSchema } from "../infrastructure/validation/schemas/answerSchemas";
...
const answerController = di.resolve(AnswerController);
...
router.post(
  ...
  auditMiddleware("ANSWER_CREATE", { targetExtractor: (req) => ({ type: "answer", id: req.body?._id }) }),
...
router.put(
  ...
  [
    ...
    auditMiddleware("ANSWER_UPDATE", { targetExtractor: (req) => ({ type: "answer", id: req.params["answer_id"] }) }),
  ],
...
router.delete(
  ...
  [
    ...
    getAccessToRoute,
    getAnswerOwnerAccess,
    auditMiddleware("ANSWER_DELETE", { targetExtractor: (req) => ({ type: "answer", id: req.params["answer_id"] }) }),
  ],
